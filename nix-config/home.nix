...
{
  imports = [
    # :: Uncomment on machines used for mobile development
    # ./mobile.nix
  ];
  ...
  # :: Neovim — binary managed by Nix, config symlinked from dotfiles
  programs.neovim = {
    enable = true;
    defaultEditor = true;
    viAlias = true;
  ...
  # :: Symlink ~/.config/nvim → dotfiles/lazyvim (keeps LazyVim writable)
  xdg.configFile."nvim".source = config.lib.file.mkOutOfStoreSymlink
    "${config.home.homeDirectory}/thanmatt-dotfiles/lazyvim";

  programs.emacs = {
    enable = true;
    package = pkgs.emacs; # :: Pure GTK version is best for Wayland/Niri
  };
  ...
  programs.fish = {
    ...
    shellAliases = {
      ...
      update-all = "sudo nixos-rebuild switch --flake ~/nix-config#my-laptop";
    ...
    interactiveShellInit = ''
      ...
      # :: Wayland + tmux environment restoration
      if set -q TMUX
